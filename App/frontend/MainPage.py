...
logger_ = ProjectLogger("frontend").get_logger()

# Initialize session state
logger_.info("Chatbot UI Starting...")
if "messages" not in st.session_state:
    ...
    logger_.info("Initialized session state with default assistant message")
...
def get_sentiment_from_api(text=None, audio_file=None):
    ...
    try:
        ...
        if text:
            data["text"] = text
            logger_.info("Sending Text Request to Backend")
        if audio_file:
            files["audio"] = (audio_file.name, audio_file, audio_file.type)
            logger_.info("Sending Audio File to Backend")

        response = requests.post(FASTAPI_URL, data=data, files=files)
        response.raise_for_status()
        result = response.json()
        logger_.info(f"Result Received from Backend: {result}")

        # API returns { "prediction": 0 or 1 }
        return "Negative" if result["prediction"] == 1 else "Positive"
    except requests.exceptions.RequestException as e:
        logger_.error(f"API request failed: {e}")
        return f"API error: {e}"
    except Exception as e:
        logger_.exception("Exception occurred when calling FastAPI")
        return f"API error: {e}"
...
def process_audio_file(audio_file):
    ...
    try:
        ...
        logger_.info(f"Audio Processed Successfully, Sentiment: {sentiment}")
...
def add_message(role, content, sentiment=None):
    ...
    logger_.debug(f"Added message | role={role} | content={content[:50]} | sentiment={sentiment}")
        ...
        for i, message in enumerate(st.session_state.messages):
            if message["role"] == "user":
                ...
                if message["sentiment"]:
                    sentiment_class = "sentiment-positive" if message["sentiment"] == "Positive" else "sentiment-negative"
...
def process_user_message(user_input):
    ...
    if not user_input.strip():
        logger_.warning("Empty user input ignored")

        return

    logger_.info(f"Processing user message: {user_input[:50]}")
    ...
    logger_.info(f"Generated bot response: {bot_response}")
...
with button_col:
    ...
    audio_file = st.file_uploader("Upload Audio File", type=['wav','mp3','m4a','ogg'], label_visibility="collapsed")
    process_audio_button = st.button("🎤 Process Audio")

# --- Handle user actions ---
if send_button and user_input.strip():
    logger_.info("Send button clicked for text input")
    process_user_message(user_input.strip())
    st.session_state.user_input = ""

if process_audio_button and audio_file is not None:
    logger_.info(f"Process audio button clicked | file: {audio_file.name}")
    ...
    with st.spinner("Processing audio..."):
        ...
        if text:
            ...
            st.error(error)
logger_.debug(f"Current session messages count: {len(st.session_state.messages)}")
st.markdown('</div>', unsafe_allow_html=True)